...
describe('Rate Limiter', () => {
  ...
  describe('checkRateLimit', () => {
    ...
    it('should track token usage correctly', async () => {
      ...
      expect(result.remaining.tokens).toBe(
        RATE_LIMIT_CONFIG.maxTokensPerDay - tokensToConsume
      )
    })
  ...
  describe('consumeRequest', () => {
    it('should increment request count', async () => {
      ...
      expect(afterCheck.remaining.requests).toBe(
        initialCheck.remaining.requests - 1
      )
    })
    ...
    it('should track multiple requests correctly', async () => {
      ...
      const result = await checkRateLimit(identifier)
      expect(result.remaining.requests).toBe(
        RATE_LIMIT_CONFIG.maxRequestsPerMin - requestsToConsume
      )
    })
  })
  ...
  describe('consumeTokens', () => {
    it('should increment token count', async () => {
      ...
      expect(afterCheck.remaining.tokens).toBe(
        initialCheck.remaining.tokens - tokensToConsume
      )
    })
    ...
    it('should track multiple token consumptions', async () => {
      ...
      const result = await checkRateLimit(identifier)
      expect(result.remaining.tokens).toBe(
        RATE_LIMIT_CONFIG.maxTokensPerDay - tokens1 - tokens2
      )
    })
  })
