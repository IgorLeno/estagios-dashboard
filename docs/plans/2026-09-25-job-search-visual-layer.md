...
- [x] **WP5 UI** — telas 1–3 sobre `JobView`.
  ...
  - Achado corrigido: prefetch dos `<Link>` (proxy renova o cookie de sessão em cada request) ressuscitava a sessão logo após "Sair"; links de dados e sidebar usam `prefetch={false}`. Risco residual (qualquer request em voo durante o logout) anotado para WP4.
