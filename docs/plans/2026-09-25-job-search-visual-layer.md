...
- [ ] **WP1 job-search: aba Dossiers** (repo job-search, PR próprio) — contrato exato de headers/schema + `dossier.py export` + parser do writeset + testes + validador + doc SA read-only. Criação da aba e da SA só depois do contrato fechado.
- [ ] **WP2 Remoção de IA/automação** — itens "Remove" acima; app fica com shell visual e dados fixture vazios; build verde.
- [ ] **WP3 Camada de dados** — sem auth só com fixtures em dev; nenhuma versão lendo a Sheet real é publicada antes do WP4 — `lib/job-search/*` + fixtures portadas de `job-search/scripts/testing_fixtures.py` (`fake-1001`) + testes unitários (parse por cabeçalho, enum inválido, `trilha` legado, SUBMIT_INTENT órfão, sha mismatch, métricas).
