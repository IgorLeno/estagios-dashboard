...
- [ ] **WP6 Local-first: sair de Vercel e Supabase** — escopo revisto pelo usuário (2026-09-25): em vez de limpar env vars no Vercel, o repositório deixa de depender de Vercel e Supabase. Exclusão dos projetos externos (Vercel, Supabase) só com autorização separada.
  - Status (2026-09-25): branch `chore/local-first-dashboard`, não publicada. Removidos `@vercel/analytics` (import, `<Analytics />`, dep, lockfile) e o `VERCEL=1` residual de `source.test.ts`. Nenhuma dependência Supabase ativa. Docs atuais descrevem o modo local-first; docs legados que parecem guias operacionais ganharam aviso de histórico.
