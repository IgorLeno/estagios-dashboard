...
- [x] **WP2 Remoção de IA/automação** — itens "Remove" acima; app fica com shell visual e dados fixture vazios; build verde.
  ...
  - Verificado: `pnpm lint` (0 erros), `pnpm format:check`, `tsc --noEmit` limpo, `pnpm test` (42/42), `pnpm build`; shell navegado via `next start` no browser pane (abas, `?tab=`, `/vaga/inexistente`). Não verificado: `pnpm test:e2e` (Chromium do Playwright 1.56.1 não instalado; `pnpm dev` com Turbopack falha por limite de arquivos da máquina — `Too many open files`). Nota: `next.config.mjs` tem `ignoreBuildErrors`, então `pnpm build` não faz type-check.
